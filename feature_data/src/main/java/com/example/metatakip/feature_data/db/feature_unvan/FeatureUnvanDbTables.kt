package com.example.metatakip.feature_data.db.feature_unvan

import android.database.sqlite.SQLiteDatabase
import android.util.Log
import java.util.UUID

object FeatureUnvanDbTables {

    const val TABLE = "unvan"
    private const val TAG = "FeatureUnvanDb"
    ...
    fun create(db: SQLiteDatabase) {
        ...
        """)
        db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS idx_unvan_uuid ON $TABLE(uuid)")
    ...
    fun upgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        ...
        try { db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS idx_unvan_uuid ON $TABLE(uuid)") } catch (_: Exception) {}
        patchMissingUuids(db)
    }
